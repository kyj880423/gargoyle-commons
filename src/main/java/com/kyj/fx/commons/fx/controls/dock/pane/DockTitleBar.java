...
package com.kyj.fx.commons.fx.controls.dock.pane;
...
public class DockTitleBar extends HBox implements EventHandler<MouseEvent> {
	...
	public DockTitleBar(DockNode dockNode) {
		...
		// [시작]클릭 이벤트 디스패처 등록.
		EventDispatcher originalEventDispater = closeButton.getEventDispatcher();
		closeButton.setEventDispatcher(new EventDispatcher() {
			@Override
			public Event dispatchEvent(Event event, EventDispatchChain tail) {

				if (MouseEvent.MOUSE_PRESSED == event.getEventType()) {
					tail.dispatchEvent(new DockClosePressEvent(MouseEvent.MOUSE_PRESSED));
				}
				originalEventDispater.dispatchEvent(event, tail);
				return event;
			}
		});
		// [끝]클릭 이벤트 디스패처 등록.
