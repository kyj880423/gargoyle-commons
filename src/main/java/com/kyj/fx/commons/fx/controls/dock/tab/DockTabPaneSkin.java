...
import com.kyj.fx.commons.fx.controls.dock.pane.DockEvent;
import com.kyj.fx.commons.fx.controls.dock.pane.DockNode;
import com.kyj.fx.commons.fx.controls.dock.pane.DockPane;
import com.kyj.fx.commons.fx.controls.dock.pane.DockPos;
...
public class DockTabPaneSkin extends BehaviorSkinBase<DockTabPane, DockTabPaneBehavior> {
	...
	private DockTab selectedTab;
	private boolean isSelectingTab;

	public DockTabPaneSkin(DockTabPane tabPane) {
		super(tabPane, new DockTabPaneBehavior(tabPane));
		...
		for (DockTab tab : getSkinnable().getTabs()) {
			addTabContent(tab);
		}
	...
	private void initializeTabListener() {
		getSkinnable().getTabs().addListener((ListChangeListener<DockTab>) c -> {
			List<DockTab> tabsToRemove = new ArrayList<>();
			List<DockTab> tabsToAdd = new ArrayList<>();
			int insertPos = -1;

			while (c.next()) {
				if (c.wasPermutated()) {
					DockTabPane tabPane = getSkinnable();
					List<DockTab> tabs = tabPane.getTabs();
					...
					DockTab selTab = tabPane.getSelectionModel().getSelectedItem();
					List<DockTab> permutatedTabs = new ArrayList<DockTab>(size);
					getSkinnable().getSelectionModel().clearSelection();

					// save and set tab animation to none - as it is not a good
					// idea
			...
			if (!tabsToAdd.isEmpty()) {
				for (TabContentRegion tabContentRegion : tabContentRegions) {
					DockTab tab = tabContentRegion.getTab();
	...
	private void removeTabContent(DockTab tab) {
		for (TabContentRegion contentRegion : tabContentRegions) {
	...
	private static class StyleableProperties {
		...
		private final static CssMetaData<DockTabPane, TabAnimation> OPEN_TAB_ANIMATION = new CssMetaData<DockTabPane, DockTabPaneSkin.TabAnimation>(
				"-fx-open-tab-animation", new EnumConverter<TabAnimation>(TabAnimation.class), TabAnimation.GROW) {

			@Override
			public boolean isSettable(DockTabPane node) {
				return true;
			}

			@Override
			public StyleableProperty<TabAnimation> getStyleableProperty(DockTabPane node) {
				DockTabPaneSkin skin = (DockTabPaneSkin) node.getSkin();
				return (StyleableProperty<TabAnimation>) (WritableValue<TabAnimation>) skin.openTabAnimation;
			}
		};

		private final static CssMetaData<DockTabPane, TabAnimation> CLOSE_TAB_ANIMATION = new CssMetaData<DockTabPane, DockTabPaneSkin.TabAnimation>(
				"-fx-close-tab-animation", new EnumConverter<TabAnimation>(TabAnimation.class), TabAnimation.GROW) {
	...
	class TabHeaderArea extends StackPane {
		...
		public TabHeaderArea() {
			...
			setManaged(false);
			final DockTabPane tabPane = getSkinnable();
		...
		private void installDocking() {
			...
			headersRegion.addEventHandler(MouseEvent.MOUSE_RELEASED, e -> {
				...
				if (MouseButton.PRIMARY == e.getButton() && e.getClickCount() == 1) {
					...
					if (!headersRegion.contains(e.getX(),
							e.getY()) /* && isMovingTarget.get() */) {
						LOGGER.debug("action");
						...
						// DockTab dockTab =
						// getSkinnable().getTabs().get(selectIdx);
						...
						if (null != getSkinnable().getTabs().remove(selectIdx)) {
							...
							DockNode dockNode = new DockTabContent(content, text);
							...
							FxUtil.createDockStageAndShow(getSkinnable().getScene().getWindow(), dockNode,
									new Point2D(e.getScreenX(), e.getScreenY()), false);
			...
			getSkinnable().addEventHandler(DockEvent.DOCK_OVER, ev -> {
				LOGGER.debug("source : {} , ev {} , content : {} ", ev.getSource(), ev, ev.getContents());
				if (!dockIndicatorOverlay.isShowing()) {
			...
			getSkinnable().addEventHandler(DockEvent.DOCK_RELEASED, ev -> {
				...
				LOGGER.debug("getsource : {} ", ev.getSource().toString());
				LOGGER.debug("intersect node : {}", ev.getPickResult().getIntersectedNode());
				// System.out.println(ev.getPickResult().getIntersectedNode());

				if (dockIndicatorOverlay.isShowing()) {
					dockIndicatorOverlay.hide();
				} else {
					if (null == ev.getPickResult().getIntersectedNode())
						return;
				...
				if (contents != null && contents instanceof DockNode) {
					...
					if (e == null) {
						...
						e.setContextMenu(
								/*
								SharedMemory.getSystemLayoutViewController().closeContextMenu()
								*/
								getSkinnable().getContextMenu()
								);
					}
		...
		private void addTab(DockTab tab, int addToIndex) {
			...
			ObservableList<Node> children = headersRegion.getChildren();

			children.add(addToIndex, tabHeaderSkin);
		}
		...
		private void removeTab(DockTab tab) {
		...
		private TabHeaderSkin getTabHeaderSkin(DockTab tab) {
		...
		private void setScrollOffset(double newScrollOffset) {
			...
			if ((visibleWidth - newScrollOffset) > offset && newScrollOffset < 0) {
				...
				// area width
				// is expanded), and if it isn't modify the scroll offset to
				// bring
		...
		protected void layoutChildren() {
			...
			positionInArea(headersRegion, startX, startY, w, h,
	...
	class TabHeaderSkin extends StackPane {
		...
		public TabHeaderSkin(final DockTab tab) {
			getStyleClass().setAll(tab.getStyleClass());
			setId(tab.getId());
			...
			closeBtn = new StackPane() {
				...
				public void executeAccessibleAction(AccessibleAction action, Object... parameters) {
					...
					case FIRE: {
						DockTab tab = getTab();
						DockTabPaneBehavior behavior = getBehavior();
			...
			inner = new StackPane() {
				@Override
				protected void layoutChildren() {
					final DockTabPane skinnable = getSkinnable();
			...
			getProperties().put(DockTab.class, tab);
		...
		private void removeListeners(DockTab tab) {
			listener.dispose();
	...
	class TabContentRegion extends StackPane {
		...
		public TabContentRegion(DockTab tab) {
	...
	class TabControlButtons extends StackPane {
		...
		public TabControlButtons() {
			...
			inner = new StackPane() {
				...
				protected double computePrefWidth(double height) {
					...
					double maxArrowWidth = !isShowTabsMenu() ? 0
							: snapSize(downArrow.prefWidth(getHeight())) + snapSize(downArrowBtn.prefWidth(getHeight()));
					pw = 0.0F;
					if (isShowTabsMenu()) {
						pw += maxArrowWidth;
					}
				...
				private void positionArrow(Pane btn, StackPane arrow, double x, double y, double width, double height) {
					...
					positionInArea(btn, x, y, width, height,
							/* baseline ignored */0, HPos.CENTER, VPos.CENTER);
			...
			tabPane.getTabs().addListener((ListChangeListener<DockTab>) c -> setupPopupMenu());
		...
		private void setupPopupMenu() {
			...
			for (final DockTab tab : getSkinnable().getTabs()) {
	...
	class TabMenuItem extends RadioMenuItem {
		...
		public TabMenuItem(final DockTab tab) {
			super(tab.getText(), DockTabPaneSkin.clone(tab.getGraphic()));
			this.tab = tab;
			setDisable(tab.isDisable());
