 ...
 *
 */
public class XMLAttibuteFormatWriter extends XMLFilterImpl implements LexicalHandler {
	...
	public XMLAttibuteFormatWriter(Writer writer) {
		this(writer, DEFAULT_FORMAT);
	}

	public XMLAttibuteFormatWriter(Writer writer, OutputFormat format) {
		this.writer = writer;
		this.format = format;
		namespaceStack.push(Namespace.NO_NAMESPACE);
	}

	public XMLAttibuteFormatWriter() {
	...
	public XMLAttibuteFormatWriter(OutputStream out) throws UnsupportedEncodingException {
		this.format = DEFAULT_FORMAT;
	...
	public XMLAttibuteFormatWriter(OutputStream out, OutputFormat format) throws UnsupportedEncodingException {
		this.format = format;
		this.writer = createWriter(out, format.getEncoding());
		this.autoFlush = true;
		namespaceStack.push(Namespace.NO_NAMESPACE);
	...
	public XMLAttibuteFormatWriter(OutputFormat format) throws UnsupportedEncodingException {
		this.format = format;
		...
		try {
			...
			if (format.isTrimText()) {
				if ((lastOutputNodeType == Node.TEXT_NODE) && !charsAdded) {
					...
					writer.write(' ');
				} else if (lastOutputNodeType == Node.ELEMENT_NODE && format.isPadText() && lastElementClosed
